...
do
        if [ "$code1" == "h264" ]; then
                encode_cmd="${SUDO} tastset -c 0-$(nproc) gst-launch-1.0 filesrc location=~/${video_src} ! videoparse width=4096 height=2160 format=nv12 framerate=60/1  ! queue ! x264enc ! queue ! perf ! fakesink -e"
        elif [ "$code1" == "h265" ]; then
                encode_cmd="${SUDO} tastset -c 0-$(nproc) gst-launch-1.0 filesrc location=~/${video_src} ! videoparse width=4096 height=2160 format=nv12 framerate=60/1  ! queue ! x265enc ! queue ! perf ! fakesink -e"
        else
                encode_cmd="${SUDO} tastset -c 0-$(nproc) gst-launch-1.0 filesrc location=~/${video_src} ! videoparse width=4096 height=2160 format=nv12 framerate=60/1 ! queue ! {code1}enc ! queue ! perf ! fakesink -e"
        fi
