...
def preprocess_MLPerf(model, checkpoint_name, folds, fp16, list_of_lists, output_filenames, preprocessing_folder, num_threads_preprocessing):
    ...
    trainer, _ = load_model_and_checkpoint_files(model, folds, fp16, checkpoint_name=checkpoint_name)
...
def preprocess_3dunet_ref(model_dir_base, preprocessed_data_dir_base):
    ...
    # Append the calibration image set together with the validation files
    calibration_files = []
    with open(calibration_fold_file) as f:
        for line in f:
            calibration_files.append(line.rstrip())
    ...
    calibration_lists = [[os.path.join(raw_data_dir, i) for i in all_files if i[:len(j)].startswith(j) and
...
def preprocess_3dunet_npy_inner(base_dir, is_calibration=False):
    """
    ...
    if not is_calibration:
        fp16_linear_dir = os.path.join(npy_preprocessed_dir, "fp16_linear")
        maybe_mkdir(fp16_linear_dir)
        fp16_dhwc8_dir = os.path.join(npy_preprocessed_dir, "fp16_dhwc8")
        maybe_mkdir(fp16_dhwc8_dir)
        int8_cdhw32_dir = os.path.join(npy_preprocessed_dir, "int8_cdhw32")
    ...
    for file in preprocessed_files:
        ...
        if not is_calibration:
            np.save(os.path.join(fp16_linear_dir, file + ".npy"), d.astype(np.float16))
            np.save(os.path.join(fp16_dhwc8_dir, file + ".npy"),
                    np.pad(d.astype(np.float16), ((0, 4), (0, 0), (0, 0), (0, 0)), mode="constant").transpose(1, 2, 3, 0))
            np.save(os.path.join(int8_cdhw32_dir, file + ".npy"),
                    preprocess_3dunet_int8_cdhw32(d.astype(np.float32)))


def preprocess_3dunet_npy(preprocessed_data_dir, is_calibration=False):
    """Convert preprocessed val and calib pickle files into npy based on data types."""
    print("Converting validation data to npy files...")
    preprocess_3dunet_npy_inner(os.path.join(preprocessed_data_dir, "brats"), False)
    print("Converting calibration data to npy files...")
    preprocess_3dunet_npy_inner(os.path.join(preprocessed_data_dir, "brats", "calibration"), True)


def preprocess_3dunet_int8_cdhw32(fp32_input):
    """
    Helper function to convert input data into int8_cdhw32 format.
    """
    ...
    return np.pad(int8_input, ((0, 28), (0, 0), (0, 0), (0, 0)), mode="constant").transpose(1, 2, 3, 0)
