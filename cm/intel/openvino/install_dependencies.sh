...
sudo apt-get update
sudo apt install -y python3-dev python3-pip unzip python3-opencv nvidia-cuda-toolkit jq python3.8-venv libssl-dev gpg-agent wget curl intel-gpu-tools 

echo ${SKIPS}
echo -e "\e[0;34m ========== Installing OpenVino Toolkit for Benchmark Tools=========== \e[0m"
echo ${SKIPS}	
if [ ! -d /opt/intel/openvino_2021 ]; then
	wget http://pgluint99.png.intel.com:8080//workload_installer/linux_benchmark/OpenVino/Installer/l_openvino_toolkit_p_2021.3.394.tgz
	tar xvf l_openvino_toolkit_p_2021.3.394.tgz
	cat ${CUR_DIR}/cm/performance/mlperf/openvino/scripts/silent.cfg > l_openvino_toolkit_p_2021.3.394/silent.cfg
...
if [ "${DIST}" == "focal" ]; then
	...
	python3 -m pip install requests --upgrade
	sudo chmod a+r /usr/lib/x86_64-linux-gnu/libcuda*
	sudo chmod a+r /usr/local/cuda-11.4/targets/x86_64-linux/lib/libcuda*
fi
...
echo ${SKIPS}
bash -E /opt/intel/openvino_2021/deployment_tools/model_optimizer/install_prerequisites/install_prerequisites.sh

echo ${SKIPS}
echo -e "\e[0;34m ========== Installing OpenVino Model Optimizer Caffe Dependencies on system =========== \e[0m"
echo ${SKIPS}
bash -E /opt/intel/openvino_2021/deployment_tools/model_optimizer/install_prerequisites/install_prerequisites.sh caffe

echo ${SKIPS}
echo -e "\e[0;34m ========== Installing OpenVino Model Optimizer ONNX Dependencies on system =========== \e[0m"
echo ${SKIPS}
bash -E /opt/intel/openvino_2021/deployment_tools/model_optimizer/install_prerequisites/install_prerequisites.sh onnx

echo ${SKIPS}
echo -e "\e[0;34m ========== Installing OpenVino Model Optimizer TensorFlow Dependencies on system =========== \e[0m"
echo ${SKIPS}
bash -E /opt/intel/openvino_2021/deployment_tools/model_optimizer/install_prerequisites/install_prerequisites.sh tf

echo ${SKIPS}
echo -e "\e[0;34m ========== Installing OpenVino Model Optimizer TensorFlow v2 Dependencies on system =========== \e[0m"
echo ${SKIPS}
bash -E /opt/intel/openvino_2021/deployment_tools/model_optimizer/install_prerequisites/install_prerequisites.sh tf2

echo ${SKIPS}
echo -e "\e[0;34m ========== Installing OpenVino Model Optimizer Keras Dependencies on system =========== \e[0m"
echo ${SKIPS}
bash -E /opt/intel/openvino_2021/deployment_tools/model_optimizer/install_prerequisites/install_prerequisites.sh kaldi

echo ${SKIPS}
echo -e "\e[0;34m ========== Installing OpenVino Model Optimizer MXNET Dependencies on system =========== \e[0m"
echo ${SKIPS}
bash -E /opt/intel/openvino_2021/deployment_tools/model_optimizer/install_prerequisites/install_prerequisites.sh mxnet
