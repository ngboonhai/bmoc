...
do
        if [ "$code1" == "h264" ]; then
                encode_cmd="${SUDO} taskset -c 0-$(nproc) gst-launch-1.0 filesrc location=~/${video_src} ! videoparse width=4096 height=2160 format=nv12 framerate=60/1 ! queue ! videoconvert ! queue ! x264enc ! queue ! perf ! fakesink -e"
