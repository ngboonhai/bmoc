...
do
		if [ "$code1" == "h264" ]; then
				...
				code=h264
				video_src="bbb_sunflower_2160p_60fps_normal.mp4"
				transcode_cmd="${SUDO} taskset -c 0-$(nproc) gst-launch-1.0 filesrc location=~/${video_src} num-buffers=$TotalFrame ! qtdemux ! queue ! ${code}parse ! queue ! avdec_${code} ! queue ! ${VIDEO_CONVERTOR} ! queue ! x265enc ! queue ! perf ! fakesink -e"
		elif [ "$code1" == "h264_vp8" ]; then
				code=h264
				video_src="bbb_sunflower_2160p_60fps_normal.mp4"
				transcode_cmd="${SUDO} taskset -c 0-$(nproc) gst-launch-1.0 filesrc location=~/${video_src} num-buffers=$TotalFrame ! qtdemux ! queue ! ${code}parse ! queue ! avdec_${code} ! queue ! ${VIDEO_CONVERTOR} ! queue ! vp8enc ! queue ! perf ! fakesink -e"
		elif [ "$code1" == "h264_vp9" ]; then
				code=h264
				video_src="bbb_sunflower_2160p_60fps_normal.mp4"
				transcode_cmd="${SUDO} taskset -c 0-$(nproc) gst-launch-1.0 filesrc location=~/${video_src} num-buffers=$TotalFrame ! qtdemux ! queue ! ${code}parse ! queue ! avdec_${code} ! queue ! ${VIDEO_CONVERTOR} ! queue ! vp9enc ! queue ! perf ! fakesink -e"
				...
				code=h265
				video_src="bbb_sunflower_2160p_60fps_normal.mkv"
				transcode_cmd="${SUDO} taskset -c 0-$(nproc) gst-launch-1.0 filesrc location=~/${video_src} num-buffers=$TotalFrame ! matroskademux ! queue ! ${code}parse ! queue ! avdec_${code} ! queue ! ${VIDEO_CONVERTOR} ! queue ! vp8enc ! queue ! perf ! fakesink -e"
		elif [ "$code1" == "h265_vp9" ]; then
				code=h265
				video_src="bbb_sunflower_2160p_60fps_normal.mkv"
				transcode_cmd="${SUDO} taskset -c 0-$(nproc) gst-launch-1.0 filesrc location=~/${video_src} num-buffers=$TotalFrame ! matroskademux ! queue ! ${code}parse ! queue ! avdec_${code} ! queue ! ${VIDEO_CONVERTOR} ! queue ! vp9enc ! queue ! perf ! fakesink -e"
		elif [ "$code1" == "vp8" ]; then
